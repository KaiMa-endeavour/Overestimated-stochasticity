...
raup_crick_abundance <- function(spXsite, plot_names_in_col1 = TRUE, null_model = "shuffle", parallel = TRUE, nworker = 4, classic_metric = FALSE, split_ties = TRUE, reps = 9999, set_all_species_equal = FALSE, as.distance.matrix = TRUE, report_similarity = FALSE) {
  require(ecodist)
  require(parallel)
  ...
  null_dist <- function(i, gamma, spXsite, spXsite.inc, null.one, null.two, occur, abundance, null_model) {
    if (null_model == "shuffle") {
      ...
      com1.int <- rep(0, gamma)
      com2.int <- rep(0, gamma)

      id <- sample(1:gamma, samp.richness[null.one], replace = F, prob = sp.freqence)
      com1.int[id] <- 1
      com1 <- rep(0, length(com1.int))
      xx <- spXsite[null.one, spXsite[null.one, ] > 0]
      com1[com1.int > 0] <- xx[sample.int(length(xx))]
      rm("id", "com1.int")

      id <- sample(1:gamma, samp.richness[null.two], replace = F, prob = sp.freqence)
      com2.int[id] <- 1
      com2 <- rep(0, length(com2.int))
      xx <- spXsite[null.two, spXsite[null.two, ] > 0]
    ...
    if (null_model == "proportional") {
      ...
      colnames(com1.sp.counts) <- "counts" # head(com1.sp.counts);
      ...
      colnames(com2.sp.counts) <- "counts" # head(com2.sp.counts);
    ...
    null_bray_curtis <- distance(null.spXsite, method = "bray-curtis")
  }
  ...
  for (null.one in 1:(nrow(spXsite) - 1)) {
    for (null.two in (null.one + 1):nrow(spXsite)) {
      if (parallel) {
        ...
        clusterEvalQ(c1, require(ecodist))
        null_bray_curtis <- unlist(parLapply(c1, 1:reps, null_dist, gamma, spXsite, spXsite.inc, null.one, null.two, occur, abundance, null_model))
        stopCluster(c1)
      } else {
        null_bray_curtis <- unlist(lapply(1:reps, null_dist, gamma, spXsite, spXsite.inc, null.one, null.two, occur, abundance, null_model))
      }
